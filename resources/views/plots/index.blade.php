...
<div class="container mx-auto p-6">
    ...
    <div class="bg-white shadow-xl rounded-2xl p-8 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center">
            <h1 class="text-3xl font-extrabold text-green-700 mb-4 md:mb-0">{{ __('Plots List') }}</h1>
            <a href="{{ route('plots.create') }}"
               class="inline-flex items-center px-5 py-2 text-white bg-green-600 rounded-xl hover:bg-green-700 shadow transition duration-300">
                <i class="fa-solid fa-plus mr-2"></i> {{ __('Add Plot') }}
            </a>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 rounded-xl">
                <thead class="bg-green-600 text-white uppercase text-sm">
                    <tr>
                        <th class="px-6 py-3 text-left">{{ __('Farmer') }}</th>
                        <th class="px-6 py-3 text-left">{{ __('Plot Size (Rai)') }}</th>
                        <th class="px-6 py-3 text-left">{{ __('Location') }}</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200 text-sm">
                    @forelse($plots as $plot)
                        <tr class="hover:bg-green-50 transition duration-200">
                            <td class="px-6 py-3 font-medium text-gray-800">{{ $plot->farmer->name ?? '-' }}</td>
                            <td class="px-6 py-3 text-gray-600">{{ $plot->plot_size_rai }}</td>
                            <td class="px-6 py-3 text-gray-600">{{ $plot->plot_location }}</td>
                            <td class="px-6 py-3 text-gray-600">{{ $plot->notes }}</td>
