...
@section('title', __('Edit Farmer'))

@section('content')
<div class="container mx-auto p-6">
    <div class="bg-white shadow-xl rounded-2xl p-8 space-y-6 max-w-2xl mx-auto">
        {{-- Page Header --}}
        <h1 class="text-3xl font-extrabold text-green-700">{{ __('Edit Farmer') }}</h1>
        ...
        <form action="{{ route('main.farmer.update', $farmer) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') ?? $farmer->email }}"
                        class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-400 focus:outline-none"
                        required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="address" value="{{ old('address') ?? $farmer->address }}"
                        class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-400 focus:outline-none">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="farm_location" value="{{ old('farm_location') ?? $farmer->farm_location }}"
                        class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-400 focus:outline-none">
                </div>

                <div>
                    <label class="block mb-2 font-semibold text-gray-700">{{ __('Farm Size (ha)') }}</label>
                    <input type="number" step="0.01" name="farm_size" value="{{ old('farm_size') ?? $farmer->farm_size }}"
                        class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-400 focus:outline-none">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="notes"
                    class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-400 focus:outline-none"
                    rows="4">{{ old('notes') ?? $farmer->notes }}</textarea>
            </div>
